...
// use a separate array to track the reference counts of physical pages
#define PA2PGREF_ID(p) (((p)-KERNBASE)/PGSIZE)
#define PGREF_MAX_ENTRIES PA2PGREF_ID(PHYSTOP)

struct spinlock pgreflock; // use a separate lock for page reference counts
int pageref[PGREF_MAX_ENTRIES]; // use an array to track the reference counts of physical pages
...
// use this macro to access the reference count of a physical page
...
kfree(void *pa)
{
  ...
  acquire(&pgreflock);
  if(--PA2PGREF(pa) <= 0) { // if reference count is 0 or less, free the page
...
kalloc(void)
{
  ...
  if(r){
    ...
    // newly allocated page's reference count is 1
    PA2PGREF(r) = 1;
  }
